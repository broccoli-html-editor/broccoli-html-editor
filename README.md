...
- 各フィールド間のデータの互換性が向上した。直接文字列でデータを格納していた `html`, `text`, `markdown`, `html_attr_text`, `href` の各フィールドが、`multitext` に合わせて `{"src": src}` の型で格納するように変更された。(読み込みについては旧来の文字列の形式との互換性が維持される)
- 内部コードの細かい修正。
